  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.92)', zIndex: 2000, display: 'flex', flexDirection: 'column', padding: '8px' }}>
      <div style={{ maxWidth: '600px', width: '100%', height: '100%', margin: '0 auto', background: '#222', borderRadius: '8px', border: '1px solid #444', color: '#eee', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {!isAudioEnabled && <div style={{ background: '#422', padding: '10px', marginBottom: '20px', borderRadius: '4px', color: '#f88', fontSize: '0.9rem' }}>éŸ³å£°ãŒOFFã®ãŸã‚ã€åEç”Ÿã•ã‚Œã¾ã›ã‚“ã€E</div>}
...
export default function App() {
  ...
  const handleResetSave = () => {
    if (window.confirm("ƒZ[ƒuƒf[ƒ^‚ğíœ‚µ‚Ü‚·‚©H")) {
      clearSaveData();
      setHasSave(false);
      setSeenEventIds([]);
      setActiveEvent(null);
      ...
      return (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
          <div style={{ ...cardStyle, maxWidth: '320px', width: '92%', background: '#fff', padding: '20px', borderRadius: '12px', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
            <h2 style={{ margin: '0 0 14px 0', color: THEME.nightBlue, textAlign: 'center', fontSize: '1.2em' }}>VNƒƒO</h2>
            <div style={{ flex: 1, overflowY: 'auto', borderTop: '1px solid #eee', borderBottom: '1px solid #eee', padding: '10px 0', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {vnBacklog.length === 0 ? (
                <div style={{ color: '#777', fontSize: '0.9em', textAlign: 'center', padding: '24px 0' }}>‚Ü‚¾ƒƒO‚Í‚ ‚è‚Ü‚¹‚ñ</div>
            ...
            <button style={{ ...buttonStyle, marginTop: '14px', background: '#666', color: 'white', width: '100%' }} onClick={() => setMenuView('main')}>–ß‚é</button>
          </div>
        </div>
      );
    }
    ...
    return (
      ...
      >
        <div style={{ ...cardStyle, maxWidth: '300px', background: '#fff', padding: '25px', borderRadius: '12px', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <h2 style={{ margin: '0 0 20px 0', color: THEME.nightBlue, textAlign: 'center', fontSize: '1.4em' }}>İ’è</h2>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #eee' }}>
            <span style={{ fontSize: '0.9em', color: '#999' }}>ŠÈˆÕƒZ[ƒuó‘Ô</span>
          ...
          <div style={{ marginTop: '25px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <button style={{ ...buttonStyle, marginTop: 0, background: '#ff5555', color: 'white', width: '100%' }} onClick={() => { audioEngine.playSfx('uiTapBottle'); if (window.confirm("ƒ^ƒCƒgƒ‹‚É–ß‚è‚Ü‚·‚©H")) { setIsMenuOpen(false); setScreen('START'); } }}>
              ƒ^ƒCƒgƒ‹‚Ö–ß‚é
            </button>
  ...
  if (screen === 'START') {
    ...
    const prologuePages = [
      "ç ‚æ¼ ã®è¡—ãƒã‚°ãƒªãƒãƒ«ã€‚è·¯åœ°ã®ä¸€è§’ã«ã€å°ã•ãªéŒ¬é‡‘è¡“åº—ã€æ˜Ÿç“¶å ‚ã€ãŒã‚ã‚‹ã€‚",
      "è‹¥åº—ä¸»ãƒŠãƒ¼ãƒ‡ã‚£ãƒ«ã¯ã€å®¢ã®ä¾é ¼ã«åˆã†å“ã‚’é¸ã³ãªãŒã‚‰ã€ä»Šæ—¥ã‚‚åº—ã‚’é–‹ãã€‚",
      "ã“ã‚Œã‹ã‚‰ã®10æ—¥é–“ã€‚å•†ã„ã‚’é‡ã­ã‚‹ä¸­ã§ã€å”åŠ›è€…ãŸã¡ã¨ã®ç¸ã‚‚å°‘ã—ãšã¤è‚²ã£ã¦ã„ãã€‚",
    ];
    mainContent = (
      <div data-testid="prologue-screen" style={{ ...containerStyle, position: 'relative' }}>
        ...
        <div style={{ zIndex: 2, position: 'relative', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          {renderAudioToggle()}
          <h1 style={{ ...titleStyle, marginBottom: '30px' }}>æ˜Ÿç“¶å ‚ã®å§‹ã¾ã‚Š</h1>
    ...
    mainContent = (
      <div data-testid="intro-screen" style={{ ...containerStyle, position: 'relative' }}>
        ...
        <div style={{ zIndex: 2, position: 'relative', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          {renderAudioToggle()}
          <h1 style={{ ...titleStyle, marginBottom: '20px' }}>{workshopState.day}æ—¥ç›®</h1>
          <div style={{ ...cardStyle, background: 'transparent', boxShadow: 'none', padding: 0, marginTop: '10px' }}>
            <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', marginBottom: '30px' }}>
              ...
              <div style={{ flex: 1 }}>
                <VNBox
                  ...
                  text={activeHeroine.greeting || `${PROTAGONIST.shortName}ã€ã“ã‚“ã«ã¡ã¯ã€‚ä»Šæ—¥ã‚‚ã‚ˆã‚ã—ããŠé¡˜ã„ã—ã¾ã™ã€‚`}
            ...
            <div style={{ ...narrativeBoxStyle, background: 'rgba(0,0,0,0.6)', color: '#fff', borderLeft: `4px solid ${THEME.brass}`, padding: '20px', marginBottom: '30px' }}>
              <p style={{ margin: '0 0 10px 0', lineHeight: '1.6' }}>æ˜Ÿç“¶å ‚ã®æœã€‚ãƒŠãƒ¼ãƒ‡ã‚£ãƒ«ã¯åº—ã‚’é–‹ãã€å®¢ã‚’è¿ãˆã‚‹æº–å‚™ã‚’æ•´ãˆã¦ã„ã‚‹ã€‚</p>
              <p style={{ margin: 0, lineHeight: '1.6' }}>ä»Šæ—¥ã¯ã©ã‚“ãªå“ãŒæ±‚ã‚ã‚‰ã‚Œã‚‹ã®ã‹ã€‚ã¾ãšã¯ç›¸æ‰‹ã®è©±ã‚’èãã¨ã“ã‚ã‹ã‚‰å§‹ã¾ã‚‹ã€‚</p>
            </div>
            <button data-testid="intro-start" onClick={handleBeginService} style={{ ...buttonStyle, width: '100%', maxWidth: '280px', marginTop: '10px' }}>å–¶æ¥­ã‚’å§‹ã‚ã‚‹</button>
          </div>
        </div>
      </div>
    );
    ...
    const resultNarrations = {
      5: "‘å¬Œ÷B¡“ú‚Í¯•r“°‚Ì—¬‚ê‚ª‚æ‚­Œ©‚¦‚Ä‚¢‚½B",
      4: "‚æ‚­‚â‚Á‚½B‹q‚Ì˜b‚ğ•·‚«æ‚èA•i‚ğ‘I‚Ôè‚Â‚«‚àˆÀ’è‚µ‚Ä‚¢‚éB",
      3: "‚Ü‚¸‚Ü‚¸‚¾B–À‚¢‚Í‚ ‚é‚ªAŸ‚Ìˆêè‚ªŒ©‚¦‚Ä‚¢‚éB",
      2: "‚à‚¤­‚µB‹q‚ÌˆÓ}‚ğ‚Â‚©‚ß‚ê‚ÎA•i‘I‚Ñ‚Í‚à‚Á‚ÆŠy‚É‚È‚éB",
      1: "É‚µ‚¢BÅ‚ç‚¸‘Šè‚Ì˜b‚ğ•·‚­‚Æ‚±‚ë‚©‚ç®‚¦‚Ä‚¢‚±‚¤B",
      0: "¡“ú‚Í‚¤‚Ü‚­‚¢‚©‚È‚©‚Á‚½B‚¾‚ªAŸ‚Ì‰c‹Æ‚Åæ‚è–ß‚¹‚éB",
    };
    ...
    mainContent = (
      <div style={{ ...containerStyle, position: 'relative' }}>
        ...
        <div style={{ zIndex: 2, position: 'relative', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          {renderAudioToggle()}
          <h1 style={{ ...titleStyle, marginBottom: '20px' }}>æ¥­å‹™å ±å‘Šæ›¸</h1>
          <div style={{ ...cardStyle, borderRadius: '8px', border: `3px double ${THEME.brass}`, background: 'rgba(244, 233, 213, 0.98)', padding: '25px', marginTop: '10px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '15px', marginTop: '20px' }}>
              ...
              <div style={{ fontSize: '1.1em', color: activeHeroine.themeColor, fontWeight: 'bold' }}>
                {activeHeroine.name}ã¨ã®çµE+{lastAffectionGain}
              </div>
            </div>
            ...
            <div style={{ 
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '0.8em', color: '#666', marginBottom: '4px' }}>è©•åˆ¤</div>
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '0.8em', color: '#666', marginBottom: '4px' }}>å£²ä¸E</div>
                <div style={{ fontSize: '1.1em', fontWeight: 'bold', color: THEME.brassDark }}>
                  {mgmt.sales}G
                </div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '0.8em', color: '#666', marginBottom: '4px' }}>æº€è¶³åº¦</div>
            ...
            <h2 style={{ margin: '10px 0', fontSize: '1.2em' }}>æœ€çµ‚è©•ä¾¡: {session.score} ç‚¹</h2>
            <p style={{ fontSize: '1em', marginBottom: '20px', color: '#666' }}>
              ä¾é ¼ {session.questions.length} ä»¶ä¸­ {correctCount} ä»¶é”æE
            </p>
            ...
            <button onClick={handleEndDay} style={{ ...buttonStyle, width: '100%', maxWidth: '240px' }}>Ÿ‚Öi‚Ş</button>
          </div>
        </div>
      </div>
    );
    ...
    mainContent = (
      <div style={{ ...containerStyle, position: 'relative' }}>
        ...
        <div style={{ zIndex: 2, position: 'relative', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          {renderAudioToggle()}
          <h1 style={{ ...titleStyle, marginBottom: '20px' }}>“úŸI—¹</h1>
          <div style={{ ...cardStyle, borderRadius: '8px', background: 'rgba(26, 42, 58, 0.95)', color: THEME.parchment, padding: '25px', marginTop: '10px' }}>
            ...
            <div style={{ ...narrativeBoxStyle, flex: '1', minWidth: '280px', marginBottom: 0, textAlign: 'left' }}>
              <p>¡“ú‚Ì‰c‹Æ‚Ìè‰‚¦‚ªAŸ‚Ìˆê•à‚É‚Â‚È‚ª‚Á‚Ä‚¢‚éB</p>
              <p>–À‚¢‚ª‚ ‚Á‚Ä‚àA‹q‚Ì˜b‚ğ•·‚¢‚Ä•i‚ğ‘I‚Ô—¬‚ê‚Í­‚µ‚¸‚Â®‚Á‚Ä‚¢‚­B</p>
              <p style={{ marginTop: '10px', color: THEME.brass, fontWeight: 'bold' }}>
                {activeHeroine.name}‚Æ‚Ì‰‚ÍA¡“ú‚ÌÏ‚İd‚Ë‚Å­‚µ‚¸‚ÂŒ`‚É‚È‚Á‚Ä‚¢‚­B
              </p>
            </div>
          </div>
          ...
          <div style={{ 
            ...
            <h3 style={{ margin: '0 0 15px 0', fontSize: '1em', color: '#666', borderBottom: '1px solid #ddd', paddingBottom: '5px' }}>æœ¬æ—¥ã®çµŒå–¶è¨˜éŒ²</h3>
            <div style={{ display: 'flex', justifyContent: 'space-around', marginBottom: '15px' }}>
               <div>å£²ä¸E <span style={{ color: THEME.brassDark, fontWeight: 'bold' }}>{mgmt.sales}G</span></div>
               <div>è©•åˆ¤: <span style={{ color: mgmt.reputation >= 0 ? THEME.oasisTeal : '#844', fontWeight: 'bold' }}>{mgmt.reputation >= 0 ? `+${mgmt.reputation}` : mgmt.reputation}</span></div>
            </div>
            
            <div style={{ textAlign: 'left', fontSize: '0.85em', color: '#444', borderTop: '1px solid #ddd', paddingTop: '15px' }}>
              <strong>ç¾åœ¨ã®å·¥æˆ¿ã®çŠ¶æ…E({workshopState.day}æ—¥ç›®çµ‚äºE</strong>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '10px' }}>
                 <div>ç·å£²ä¸E <span style={{ color: THEME.brassDark, fontWeight: 'bold' }}>{workshopState.sales}G</span></div>
                 <div>ç·è©•åˆ¤: <span style={{ color: workshopState.reputation >= 0 ? THEME.oasisTeal : '#844', fontWeight: 'bold' }}>{workshopState.reputation >= 0 ? `+${workshopState.reputation}` : workshopState.reputation}</span></div>
                 <div>æº€è¶³åº¦: <span style={{ color: workshopState.satisfaction >= 0 ? THEME.oasisTeal : '#844', fontWeight: 'bold' }}>{workshopState.satisfaction >= 0 ? `+${workshopState.satisfaction}` : workshopState.satisfaction}</span></div>
                 <div>è¦ªå¯Eº¦: <span style={{ color: THEME.brassDark, fontWeight: 'bold' }}>{affection[activeHeroine.id]} / 100</span></div>
              </div>
            </div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center' }}>
            <button onClick={handleNextDay} style={{ ...buttonStyle, width: '100%', maxWidth: '280px', margin: 0 }}>æ¬¡ã®æ—¥ã¸é€²ã‚€</button>
            <button onClick={handleBackToTitle} style={{ ...buttonStyle, background: THEME.nightBlue, color: THEME.sand, border: `2px solid ${THEME.brass}`, width: '100%', maxWidth: '280px', margin: 0 }}>ƒ^ƒCƒgƒ‹‚Ö–ß‚é</button>
          </div>
        </div>
      </div>
    </div>
    ...
    mainContent = (
      <div style={containerStyle}>
        {renderThemeStyles()}
        {renderAudioToggle()}
        <h1 style={titleStyle}>è¦ªæ„›ãEè¨˜éŒ²Eš{activeEvent.title}</h1>
    ...
    mainContent = (
      <div style={{ ...containerStyle, padding: 0 }}>
        ...
        <h1 style={{ ...titleStyle, display: 'none' }}>æ€ã„å‡ºã®è¨˜éŒ²</h1>
        <div style={{ ...cardStyle, maxWidth: '800px', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', margin: '0 8px 8px', width: 'calc(100% - 16px)', overflow: 'hidden' }}>
          <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', paddingRight: '2px' }}>
            {seenEvents.length === 0 ? (
              <div style={{ padding: '60px 20px', color: '#666', fontStyle: 'italic', textAlign: 'center' }}>
                <p>‚Ü‚¾Œ©•Ô‚µ‚½‚¢‹L‰¯‚Í‚ ‚è‚Ü‚¹‚ñB</p>
                <p style={{ fontSize: '0.9em', marginTop: '10px' }}>‰c‹Æ‚ği‚ß‚é‚ÆA‚±‚±‚É‹L‰¯‚ªÏ‚İã‚ª‚Á‚Ä‚¢‚«‚Ü‚·B</p>
              </div>
            ) : (
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
                        {heroineSeenEvents.map(event => (
                          ...
                          >
                            ...
                            <span style={{ fontSize: '0.8em', color: THEME.brassDark }}>Ú×‚ğŒ©‚é</span>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <div style={{ display: 'none' }}>
            <button onClick={handleBackToTitle} style={{ ...buttonStyle, background: THEME.nightBlue, color: THEME.sand, border: `2px solid ${THEME.brass}`, width: '100%', maxWidth: '240px' }}>è¨˜éŒ²ã‚’é–‰ã˜ã‚‹</button>
          </div>
        </div>
      </div>
    );
    ...
    mainContent = (
      <div data-testid="heroine-select-screen" style={containerStyle}>
        ...
        <h1 style={{ ...titleStyle, marginBottom: '20px' }}>èª°ã¨ã®ç¸ã‚’æ·±ã‚ã‚‹EE</h1>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center', marginBottom: '15px' }}>
             ...
             <div style={{ textAlign: 'left', flex: 1 }}>
               ...
               <div style={{ fontSize: '0.85em', color: '#666', marginTop: '4px' }}>
                 è¦ªå¯Eº¦: <span style={{ fontWeight: 'bold', color: THEME.textDark }}>{affection[selectedHeroine.id]}</span>
               </div>
             </div>
          </div>
          ...
          >
            {selectedHeroine.name}ã‚’é ¼ã‚E          </button>
        </div>
        ...
        <div style={{ 
          ...
          maxWidth: '350px' 
            ...
            <button onClick={handleBackToTitle} style={{ ...buttonStyle, flex: 1, margin: 0, fontSize: '0.9em', background: THEME.nightBlue, color: THEME.sand, border: `1px solid ${THEME.brass}` }}>ƒ^ƒCƒgƒ‹‚Ö–ß‚é</button>
           <button onClick={() => setScreen('MEMORIES')} style={{ ...buttonStyle, flex: 1, margin: 0, fontSize: '0.9em', background: THEME.nightBlue, color: THEME.sand, border: `1px solid ${THEME.brass}` }}>æ€ã„å‡ºã®è¨˜éŒ²</button>
        </div>
      </div>
    );
    ...
    mainContent = (
      <div style={containerStyle}>
        {renderThemeStyles()}
        {renderAudioToggle()}
        <h1 style={titleStyle}>10æ—¥é–“ãEç·æ±ºç®E</h1>
        <div style={{ ...cardStyle, border: `3px double ${THEME.brass}`, padding: '25px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '15px', marginBottom: '30px' }}>
             <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #ddd', paddingBottom: '8px' }}>
               <span>ç·å£²ä¸Šåˆè¨E</span>
               <span style={{ fontWeight: 'bold' }}>{finalSales} G</span>
             </div>
             <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #ddd', paddingBottom: '8px' }}>
               <span>æœ€çµ‚çš„ãªè©•åˆ¤</span>
               <span style={{ fontWeight: 'bold', color: finalReputation >= 0 ? THEME.oasisTeal : '#844' }}>
                 {finalReputation >= 0 ? `+${finalReputation}` : finalReputation}
               </span>
             </div>
             <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #ddd', paddingBottom: '8px' }}>
               <span>{activeHeroine.name} ã¨ã®çµE</span>
               <span style={{ fontWeight: 'bold', color: THEME.brassDark }}>{finalAffection} / 100</span>
             </div>
          </div>

          <p style={{ fontStyle: 'italic', color: '#666', fontSize: '0.95em', marginBottom: '30px', lineHeight: '1.6' }}>10“úŠÔ‚Ì‰c‹Æ‚ğ’÷‚ß‚­‚­‚èAŸ‚Ìˆê•à‚Öi‚İ‚Ü‚·B</p>

          <button onClick={handleSeeEnding} style={{ ...buttonStyle, width: '100%', maxWidth: '280px' }}>Œ‹––‚ğŒ©“Í‚¯‚é</button>
        </div>
      </div>
    );
    ...
    mainContent = (
      <div style={{ ...containerStyle, position: 'relative' }}>
        ...
        <div style={{ zIndex: 2, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <button onClick={handleFinishGame} style={{ ...buttonStyle, marginBottom: '20px', width: '100%', maxWidth: '240px' }}>ã‚¿ã‚¤ãƒˆãƒ«ã¸æˆ»ã‚E</button>
        </div>
      </div>
    );
    ...
    mainContent = (
      <div data-testid="quiz-screen" style={containerStyle}>
        ...
        <header style={{ 
          ...
          <span style={{ fontSize: '0.9em' }}>é‘‘å®šä¾é ¼ {session.currentIndex + 1} / {session.questions.length}</span>
          <span style={{ fontWeight: 'bold', color: THEME.brass }}>å ±é…¬è¦‹è¾¼: {session.score} G</span>
        </header>
    ...
    <div ref={outerWrapperRef} style={outerWrapperStyle}>
      <div style={canvasContainerStyle}>
        <div style={canvasStyle}>
          {isInitialLoading && renderLoadingOverlay("æ˜Ÿç“¶å ‚ã‚’é–‹åº—ä¸­...")}
          {isHeroineLoading && renderLoadingOverlay(`${HEROINES.find(h => h.id === previewHeroineId)?.name}ã‚’å¾E£ã¦ãE¾ãE..`)}
          ...
          {!isInitialLoading && (
            <div key={screen} className="screen-enter">
              {mainContent || (
                <div style={containerStyle}>
                  <p>Loading...</p>
                  <button onClick={handleBackToTitle} style={buttonStyle}>ã‚¿ã‚¤ãƒˆãƒ«ã¸æˆ»ã‚E</button>
                </div>
              )}
            </div>
          )}
